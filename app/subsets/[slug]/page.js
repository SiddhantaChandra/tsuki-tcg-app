...
import { getSubsetBySlug, getCardsBySubset } from '../../../lib/database'

export default function SubsetPage({ params }) {
  ...
  useEffect(() => {
    async function loadSubsetData() {
      try {
        const [subsetData, cardsData] = await Promise.all([
          getSubsetBySlug(unwrappedParams.slug),
          getCardsBySubset(unwrappedParams.slug)
        ])
  ...
  useEffect(() => {
    async function applyFilters() {
      try {
        const filteredData = await getCardsBySubset(unwrappedParams.slug, filters)
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-neutral-900 transition-colors duration-300 pt-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center justify-center h-64">
            <div className="text-gray-900 dark:text-white text-lg">Loading subset details...</div>
          </div>
        </div>
      </div>
    )
        ...
        <div className="flex">
          ...
          <div className="flex-1 px-4 sm:px-6 lg:px-8 py-8">
            ...
            <nav className="flex mb-8 text-sm">
              ...
              <span className="mx-2 text-gray-400">&gt;</span>
              <Link href={`/sets/${subset.sets?.slug}`} className="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">
                {subset.sets?.name}
              </Link>
              <span className="mx-2 text-gray-400">&gt;</span>
              <span className="text-gray-900 dark:text-white">{subset.name}</span>
            </nav>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center space-x-4 text-sm text-gray-600 dark:text-gray-400">
                    <span>Set: {subset.sets?.name}</span>
                    <span>•</span>
                    <span>Category: {subset.sets?.categories?.name}</span>
                    <span>•</span>
                    <span>{filteredCards.length} cards found</span>
                  </div>
                  {subset.release_date && (
                    <div className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                      Released: {new Date(subset.release_date).toLocaleDateString()}
                    </div>
                  )}
                </div>
